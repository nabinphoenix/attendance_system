...
function Notice({ message, tone = "success" }: { message: string; tone?: "success" | "error" }) {
  return <div role={tone === "error" ? "alert" : "status"} className={`rounded-xl border p-3 text-sm ${tone === "error" ? "border-red-500/30 bg-red-500/10 text-red-600 dark:text-red-300" : "border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300"}`}>{message}</div>;
}
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(20rem,.72fr)]">
      <div className="space-y-6">
        <section className="panel p-5 sm:p-6">
          <h2 className="mb-4 text-lg font-semibold">Profile photo</h2><div className="flex flex-col items-center gap-4 text-center sm:flex-row sm:items-center sm:text-left"><ProfileAvatar name={user.name} src={user.avatar_url} className="h-20 w-20 text-2xl" /><div className="min-w-0 flex-1"><p className="app-caption mt-1 text-sm">Use a clear photo so your classmates and team can recognize you.</p></div><label className="inline-flex min-h-11 w-full shrink-0 cursor-pointer focus-within:ring-2 focus-within:ring-emerald-400 sm:w-auto items-center justify-center rounded-lg border border-emerald-600 bg-emerald-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-emerald-700"><input className="sr-only" type="file" accept="image/jpeg,image/jpg,image/png,image/webp" onChange={uploadPhoto} disabled={uploading} />{uploading ? "Uploading…" : "Upload photo"}</label></div>
          <p className="app-caption mt-4 text-center text-xs sm:text-left">JPG, PNG, or WEBP · Maximum file size 5 MB</p>
        ...
        <form onSubmit={saveProfile} className="panel p-5 sm:p-6">
          ...
          <div className="mt-5 flex flex-col sm:flex-row sm:justify-end"><Button type="submit" loading={savingProfile}>{savingProfile ? "Saving…" : "Save profile"}</Button></div>
        </form>
        ...
        <form onSubmit={savePassword} className="panel p-5 sm:p-6">
          ...
          <div className="mt-5 flex flex-col sm:flex-row sm:justify-end"><Button type="submit" loading={savingPassword}>{savingPassword ? "Updating…" : "Change password"}</Button></div>
        </form>
      </div>
