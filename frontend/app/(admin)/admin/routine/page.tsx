...
export default function Page() {
  ...
  const totalPages = Math.max(1, Math.ceil(routinePage.total / pageSize));
  const from = routinePage.total ? (routinePage.page - 1) * pageSize + 1 : 0;
  const to = Math.min(routinePage.page * pageSize, routinePage.total);
  const firstPage = Math.max(1, Math.min(routinePage.page - 2, totalPages - 4));
