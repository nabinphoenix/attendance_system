...
export default function Page() {
  ...
  const decisionTable = (
    ...
    <table><thead><tr><th>Hold</th><th>Student</th><th>From</th><th>Manual override</th><th>Previewed result</th></tr></thead><tbody>{preview.students.map((student) => <tr key={student.id}>
      <td><input type='checkbox' checked={holds.includes(student.id)} onChange={() => { setHolds(holds.includes(student.id) ? holds.filter((id) => id !== student.id) : [...holds, student.id]); markDirty(); }} /></td>
      ...
      <td><select className={fieldClass} value={overrides[student.id] ?? ''} onChange={(event) => { const next = { ...overrides }; if (event.target.value) next[student.id] = event.target.value; else delete next[student.id]; setOverrides(next); markDirty(); }}><option value=''>Use strategy</option>{targetSections.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></td>
  ...
  return <div className='max-w-7xl'>
    ...
    <section className='panel p-5'>
      ...
      <form onSubmit={createSemester} className='mt-4 grid gap-3 md:grid-cols-5'>
        <select className={fieldClass} required value={semesterForm.batch_id} onChange={(event) => setSemesterForm({ ...semesterForm, batch_id: event.target.value, batch_level_id: '', semester_number: '' })}><option value=''>Batch</option>{batches.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
    ...
    <section className='panel mt-7 p-5'>
      ...
      <form onSubmit={previewPlacement}>
        <select className={`${fieldClass} mt-4 max-w-xl`} required value={placementSemesterId} onChange={(event) => setPlacementSemesterId(event.target.value)}><option value=''>Target Semester</option>{semesters.map((item) => <option key={item.id} value={item.id}>{semesterLabel(item)}</option>)}</select>
    ...
    <section className='panel mt-7 p-5'>
      ...
      <form onSubmit={previewPromotion}>
        <div className='mt-4 grid gap-3 md:grid-cols-2'>
          <select className={fieldClass} required value={sourceId} onChange={(event) => { setSourceId(event.target.value); setTargetId(''); }}><option value=''>Source Semester</option>{semesters.filter((item) => item.semester_number < 6).map((item) => <option key={item.id} value={item.id}>{semesterLabel(item)}</option>)}</select>
    ...
    <section className='panel mt-7 p-5'>
      ...
      <form onSubmit={moveStudent} className='mt-4 grid gap-3 md:grid-cols-5'>
        <select className={fieldClass} required value={move.student_id} onChange={(event) => void loadHistory(event.target.value)}><option value=''>Student</option>{students.map((item) => <option key={item.id} value={item.id}>{item.roll_number} - {item.name}</option>)}</select>
        <select className={fieldClass} required value={move.cohort_semester_id} onChange={(event) => setMove({ ...move, cohort_semester_id: event.target.value, target_section_id: '', effective_date: '' })}><option value=''>Semester</option>{semesters.map((item) => <option key={item.id} value={item.id}>{semesterLabel(item)}</option>)}</select>
        <select className={fieldClass} required value={move.target_section_id} onChange={(event) => setMove({ ...move, target_section_id: event.target.value })}><option value=''>Target Section</option>{moveSections.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
        <input className={fieldClass} required type='date' min={moveSemester?.start_date} max={moveSemester?.end_date} value={move.effective_date} onChange={(event) => setMove({ ...move, effective_date: event.target.value })} />
