...
export default function QRDisplay({ value, size = 340 }: { value: string; size?: number }) {
  ...
  return <>
    <button type="button" onClick={() => setOpen(true)} aria-label="Open full-screen attendance QR code" className="group mx-auto block w-full max-w-[460px] rounded-xl bg-white p-4 text-left shadow-xl shadow-black/20 transition hover:-translate-y-0.5 hover:ring-4 hover:ring-emerald-400/50 focus-visible:ring-4 focus-visible:ring-emerald-400 sm:p-6">
      <QRCodeSVG value={value} size={size} title="Attendance QR code" {...qrProps} className="h-auto w-full" />
      <span className="mt-3 flex items-center justify-center gap-2 text-sm font-semibold text-slate-700"><svg aria-hidden="true" viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2"><path d="M8 3H3v5m13-5h5v5M3 16v5h5m13-5v5h-5" /></svg>Open large-distance scan view</span>
    </button>
    {open && <div role="dialog" aria-modal="true" aria-label="Full-screen attendance QR code" className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/95 p-4 backdrop-blur-sm">
      ...
      <div className="w-full max-w-[min(86rem,calc(100vh-5.5rem),calc(100vw-2rem))] rounded-2xl bg-white p-3 shadow-2xl sm:p-4"><QRCodeSVG value={value} size={1400} title="Attendance QR code" {...qrProps} className="h-auto w-full" /><p className="mt-3 text-center text-sm font-semibold text-slate-700">High-contrast long-distance scan view</p></div>
    </div>}
  </>;
}
