...
class ImportRowResult(BaseModel):row_number:int;status:str;message:str;data:dict[str,Any]=Field(default_factory=dict)
class ImportJobRead(BaseModel):id:int;file_name:str;upload_type:str;total_rows:int;success_count:int;failed_count:int;pending_section_references:int=0;errors:list[ImportError];results:list[ImportRowResult]=Field(default_factory=list);created_at:datetime
