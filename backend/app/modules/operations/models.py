...
class ImportJob(Base):
    __tablename__="import_jobs";id:Mapped[int]=mapped_column(primary_key=True);uploaded_by:Mapped[int]=mapped_column(ForeignKey("users.id"));file_name:Mapped[str]=mapped_column(String(255));upload_type:Mapped[str]=mapped_column(String(30));total_rows:Mapped[int]=mapped_column(Integer);success_count:Mapped[int]=mapped_column(Integer);failed_count:Mapped[int]=mapped_column(Integer);errors_json:Mapped[str]=mapped_column(Text,default="[]");created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),server_default=func.now())
class AuditLog(Base):
