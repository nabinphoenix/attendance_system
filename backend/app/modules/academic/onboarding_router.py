...
from .models import InvitationStatus, Section, Student, StudentInvitation
...
def send_invitations(payload:InvitationRequest,actor:Annotated[User,Depends(require_role("admin"))],db:DbSession):
    ...
    requested=0;sent=0;registered=0;failed=0;errors=[]
    for student in db.scalars(q).all():
        requested+=1
        try:
            if student.user_id:registered+=1;continue
            if not student.email:raise ValueError("Student has no email address")
            ...
            invite=StudentInvitation(student_id=student.id,token_hash=digest,status=InvitationStatus.SENT,expires_at=datetime.now(UTC)+timedelta(hours=settings.invitation_expire_hours));db.add(invite);db.flush()
            url=f"{settings.frontend_url.rstrip('/')}/activate?token={token}"
            queue_notification(db,"student",student.id,"Activate your AntimBench account",f"Hello {student.name or 'Student'}, activate your account by opening this link before it expires: {url}","student_invitation",invite.id)
            sent+=1
        except Exception as exc:failed+=1;errors.append({"student_id":student.id,"message":str(exc)})
    log_audit(db,actor.id,"student.invitations_sent","student_invitation",0,None,{"requested":requested,"sent":sent,"already_registered":registered,"failed":failed});db.commit()
    return {"requested":requested,"sent":sent,"already_registered":registered,"failed":failed,"errors":errors}
