    ...
    for student in db.scalars(select(Student)).all():
        for stat in subject_stats(db,student.id):
            if stat["total"]>=settings.minimum_observations and stat["percentage"]<settings.attendance_threshold_percent:
                case=db.scalar(select(StudentCase).where(StudentCase.student_id==student.id,StudentCase.scope_type=="SUBJECT",StudentCase.scope_id==stat["subject_id"],StudentCase.status.in_([CaseStatus.OPEN,CaseStatus.IN_PROGRESS])));result.append(AtRiskStudent(student_id=student.id,student_name=student.user.name,subject_id=stat["subject_id"],subject_name=stat["subject_name"],attendance_percentage=stat["percentage"],observations=stat["total"],case_status=case.status.value if case else None))
    return result
