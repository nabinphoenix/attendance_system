...
def assign(id:int,p:Assignment,user:Staff,db:DbSession):
    case=db.get(StudentCase,id)
    if not case:raise HTTPException(404,"Case not found")
    case.assigned_to=p.assigned_to;db.commit();db.refresh(case);return case
...
def change_status(id:int,p:StatusChange,user:Staff,db:DbSession):
    ...
    case.status=new;case.closed_at=datetime.now(UTC) if new==CaseStatus.CLOSED else None;db.commit();db.refresh(case);return case
